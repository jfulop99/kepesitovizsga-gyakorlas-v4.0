...
public class ListByApplicantsPersonal implements ApplicantListGenerator {
    ...
    public List<Applicant> getListFromDatabase(DataSource dataSource) {
        ...
        try (Connection conn = dataSource.getConnection();
             PreparedStatement ps = conn.prepareStatement("select first_name, last_name, phone_number, email from applicants order by id")){
        ...
        try (ResultSet rs = ps.executeQuery()){
            while (rs.next()) {
                result.add(new Applicant(rs.getNString("first_name"), rs.getNString("last_name"),
                        rs.getNString("phone_number"), rs.getNString("email")));
            }
            return result;
        }
    }
